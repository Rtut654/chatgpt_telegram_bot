...
async def start_handle(update: Update, context: CallbackContext):
    ...
    reply_text = (
        "Hi! I'm <b>ChatGPT</b> bot implemented with OpenAI API 🤖\n\n"
    )
    ...
    async with user_semaphores[user_id]:
        if (
            ...
            if (
                ...
                and current_model != 'gpt-4-vision-preview'
            ):
                current_model = 'gpt-4o'
        ...
        try:
            ...
            await update.message.reply_text(
                '✅ Canceled', parse_mode=ParseMode.HTML
            )
        else:
            pass
        finally:
...
async def generate_image_handle(
    update: Update, context: CallbackContext, message=None
):
    await register_user_if_not_exists(
        update, context, update.message.from_user
...
def get_chat_mode_menu(page_index: int):
    ...
    text = (
        f'Select <b>chat mode</b> ({len(config.chat_modes)} modes available):'
    )

    # buttons
    chat_mode_keys = list(config.chat_modes.keys())
    page_chat_mode_keys = chat_mode_keys[
        page_index * n_chat_modes_per_page : (page_index + 1)
...
async def show_chat_modes_callback_handle(
    update: Update, context: CallbackContext
):
    await register_user_if_not_exists(
        update.callback_query, context, update.callback_query.from_user
    )
    if await is_previous_message_not_answered_yet(
...
SUBSCRIBE_OR_FREE = [
    [InlineKeyboardButton('🔐 Subscribe', callback_data='products')],
    [
        InlineKeyboardButton(
            '🍉 Get messages for free', callback_data='invite_friend'
        )
    ],
]
...
async def get_balance_state(chat_id, is_premium, daily_messages):
    quota: int = DAILY_QUOTA['premium'] if is_premium else DAILY_QUOTA['free']
    balance_state: str = f'<b> Your remained daily quotа is {max(0, quota - daily_messages)} messages.</b>'
...
async def show_balance_handle(update: Update, context: CallbackContext):
    ...
    balance_state = await get_balance_state(
        user_id, is_premium, daily_messages
    )
...
async def post_init(application: Application):
    from src_bot.bot.enums import CommandEnum
    ...
    await application.bot.set_my_commands(
        [
            ...
            *[
                BotCommand(
                    command=f'/{commands.command}',
                    description=commands.description,
                )
                for commands in CommandEnum
            ],
        ]
    )
...
def run_bot() -> None:
    ...
    application.add_handler(
        MessageHandler(
            filters.VIDEO & ~filters.COMMAND & user_filter,
            unsupport_message_handle,
        )
    )
    application.add_handler(
        MessageHandler(
            filters.Document.ALL & ~filters.COMMAND & user_filter,
            unsupport_message_handle,
        )
    )
    application.add_handler(
    ...
    add_handlers(application=application, filters=user_filter)
